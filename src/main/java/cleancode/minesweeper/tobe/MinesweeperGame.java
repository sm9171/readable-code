...
public class MinesweeperGame {
    ...
    private static void actOnCell(String cellInput, String userActionInput) {
        ...
        if (doesUserChooseToOpenCell(userActionInput)) {
            if (isLandMines(selectedRowIndex, selectedColIndex)) {
    ...
    private static boolean isLandMines(int selectedRowIndex, int selectedColIndex) {
    ...
    private static boolean doewUserWinTheGame() {
        return gameStatus == 1;
    }
    ...
    private static void initializeGame() {
        ...
        for (int row = 0; row < BOARD_ROW_SIZE; row++) {
            for (int col = 0; col < BOARD_COL_SIZE; col++) {
                int count = 0;
                if (!isLandMines(row, col)) {
                    if (row - 1 >= 0 && col - 1 >= 0 && isLandMines(row - 1, col - 1)) {
                        count++;
                    }
                    if (row - 1 >= 0 && isLandMines(row - 1, col)) {
                        count++;
                    }
                    if (row - 1 >= 0 && col + 1 < BOARD_COL_SIZE && isLandMines(row - 1, col + 1)) {
                        count++;
                    }
                    if (col - 1 >= 0 && isLandMines(row, col - 1)) {
                        count++;
                    }
                    if (col + 1 < BOARD_COL_SIZE && isLandMines(row, col + 1)) {
                        count++;
                    }
                    if (row + 1 < BOARD_ROW_SIZE && col - 1 >= 0 && isLandMines(row + 1, col - 1)) {
                        count++;
                    }
                    if (row + 1 < BOARD_ROW_SIZE && isLandMines(row + 1, col)) {
                        count++;
                    }
                    if (row + 1 < BOARD_ROW_SIZE && col + 1 < BOARD_COL_SIZE && isLandMines(row + 1, col + 1)) {
                        count++;
                    }
    ...
    private static void showGameStartComements() {
        System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
